...
<script language="javascript" type="text/javascript">
	function ShowPrintableVersion(id)
	{
		window.open('invoice_print.php?id='+id, "", "");
	}
</script>
{/literal}
...
Ext.onReady(function () {
	...
	var grid = new Ext.ux.webta.GridViewer({
	    ...
	    // Columns
        columns:[
			{header: _t.colId, width: 15, dataIndex: 'custom_id', sortable: true},
			{header: _t.colOrderId, width: 15, dataIndex: 'order_id', sortable: true},
			{header: _t.colIssuedFor, width: 50, dataIndex: 'description', sortable: false},
			{header: _t.colAmount+', '+currency, width: 15, dataIndex: 'total', renderer: renderTotal, sortable: true},
			{header: _t.colCreateDate, width: 20, dataIndex: 'dtcreated', sortable: true},
			{header: _t.colStatus, width: 15, dataIndex: 'status', renderer: renderStatus, sortable: true},
			{header: _t.colGate, width: 15, dataIndex: 'gate', sortable: false}
		],
    	// Row menu
	   	rowOptionsMenu: [
			{id: "option.pay",			text: _t.pay, 			href: "checkout.php?invoices[]={id}"},
			{id: "option.reject",		text: _t.reject,	 	href: "inv_view.php?task=delete&id={id}"},
			{id: "option.print",		text: _t.viewPrintable,	href: "inv_print.php?id={id}"}			
     	],
     	getRowOptionVisibility: function (item, record) {
         	if (item.id == "option.print") {
         		return true;
         	} else {
		...
		selCheckboxRenderer: function (id, p, record) {
			return record.data.status == 0 ? '<input type="checkbox" name="id[]" value="'+id+'"/>' : '';
		},
		withSelected: [
			{text: _t.makePayment, value: 'pay'}
		] 
    });
    grid.render();
    store.load();
