...
<html>
<head>
	<title>{t}Registrant Control Panel{/t}{if $title}: {$title|strip_tags}{/if}</title>
	...
	<td><table border="0" width="100%" cellpadding="0" cellspacing="0" class="Header">
		<tr>
			...
			<td width="200" nowrap valign="middle">
				...
				<input id="search_button" type="submit"  value="{t}Search{/t}" class="btn" style="margin-top:2px; margin-left: 3px;" />
				</form>
			</td>
			...
			<td align="right">
				<span style="padding-right:20px">{t}Balance{/t}: <strong>{$currency}{$balance|number_format:2}</strong></span>
				<input type="button" value="{t}Logout{/t}" onClick="document.location='login.php?logout=1'" class="btn" /></td>
		</tr>
		</table></td>
	<td width="7" class="TableHeaderCenter"></td>
</tr>
...
<table width="100%" height="100%" cellpadding="5" cellspacing="0">
  ...
  <tr>
	<td width="76%" valign="top"><table width="100%" cellspacing="0" cellpadding="0">
	  ...
	  <tr>
        <td id="message-holder">
        	{if $reg_incomplete > 0}
        		<div class="Webta_WarnMsg"><a style="color:#cf650e;" href="incomplete_orders.php?op={$smarty.class_const.INCOMPLETE_OPERATION.DOMAIN_CREATE}">{t n=$reg_incomplete}You have %1 incomplete domain registrations{/t}</a></div>
        	{/if}
        
        	{if $trans_incomplete > 0}
        		<div class="Webta_WarnMsg"><a style="color:#cf650e;" href="incomplete_orders.php?op={$smarty.class_const.INCOMPLETE_OPERATION.DOMAIN_TRANSFER}">{t n=$trans_incomplete}You have %1 incomplete domain transfers{/t}</a></div>
        	{/if}

        	{if $transout_incomplete > 0}
        		<div class="Webta_WarnMsg"><a style="color:#cf650e;" href="incomplete_orders.php?op={$smarty.class_const.INCOMPLETE_OPERATION.DOMAIN_OUTGOING_TRANSFER}">{t n=$transout_incomplete}You have %1 incomplete outgoing transfers{/t}</a></div>
        	{/if}        	
        	
        	
        	{if $trade_incomplete > 0}
        		<div class="Webta_WarnMsg"><a style="color:#cf650e;" href="incomplete_orders.php?op={$smarty.class_const.INCOMPLETE_OPERATION.DOMAIN_TRADE}">{t n=$trade_incomplete}You have %1 incomplete domain trades{/t}</a></div>
        	{/if}
			...
			{/if}
		<a name="top"></a>
			{if $help && $smarty.const.CF_USERS_INLINE_HELP == 1}
				{literal}
				...
				function CloseHelp()
				{
					...
					var url = '/client/server/misc.php?'+params
