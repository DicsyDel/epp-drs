		...
		<tr class="th">
			<td colspan="{if $colspan}{$colspan}{else}14{/if}"><table border="0" width="100%" class="WebtaTable_Footer">
					<tr>
						...
						{if $edit_page}
							<input {if $disable_save_button}disabled{/if} style="vertical-align:middle;" name="Submit" type="submit" class="btn" value="{t}Save{/t}">
							<input name="id" type="hidden" id="id" value="{$id}">
						{elseif $search_page}
							<input type="submit" class="btn" value="Search">
						{elseif $page_data_options_add}
							<a href="{$smarty.server.PHP_SELF|replace:"view":"add"}{$page_data_options_add_querystring}">{t}Add new{/t}</a>
						{/if}
						{if $next_page}
								&nbsp;<input type="submit" class="btn" name="next" value="Next" />	
						{/if}
						{if $button_js}
								&nbsp;<input id="button_js" style="vertical-align:middle;" type="button" onclick="{$button_js_action}" class="btn" name="cbtn_2" value="{$button_js_name}" />	
						{/if}
						{if $button2}
						...
						<td width="10%" align="right" nowrap>
							...
							{t}Selected:{/t}
								...
								<input type="submit" name="actionsubmit" style="vertical-align:middle;" value="{t}Apply{/t}" class="btn">
							{/if}
