	...
	{include file="client/inc/table_header.tpl"}
	   <input type="hidden" name="stepno" value="{$stepno}" />
	    ...
	    {php}
	    	...
	    	$this->assign('intable_header',_("Bulk registration &mdash; Step 2 (Checking availability of domains)"));
	    {/php}
			...
			document.observe("dom:loaded", function () {
				...
				var dc = new DomainChecker({
    	   		// {/literal}
	    	   		...
	    	   		queue: 			{$domains},
	    	   		baseUrl: 		"server/check.php?action=register",
	    	   		availMessage: 	"{t}Available for registration{/t}",
	    	   		unavailMessage: "{t}Taken{/t}",
	    	   		failedMessage:	"{t}Cannot check domain availability. Make sure that you spelled domain name correctly{/t}",
