<?php
	...
	class AfiliasRegistryModule extends GenericEPPRegistryModule 
	{
		...
		public function RunTest ($DF)
		{
			...
			$Runner->Run(new AfiliasRegistryModule_OteTestSuite(), $DF);
		}		
		...
		protected function BeforeRequest($command, &$data, $method /* args */)
		{
			...
			switch ($command)
			{
				case 'domain-check':
					...
					// TODO: IDN
					/*
					// Process IDN extension
					if ($this->RegistryAccessible->IsIDNHostName($Domain->GetHostName()))
					{
						$extension .= '<idn:check'
								 	. ' xmlns:idn="urn:ietf:params:xml:ns:idn-1.0"'
									. ' xsi:schemaLocation="urn:ietf:params:xml:ns:idn-1.0 idn-1.0.xsd">'
									//. '<idn:script>'.$Domain->{'idn-script'}.'</idn:script>'
									. '<idn:script>sv</idn:script>'
						 			. '</idn:check>';						
					}
					*/
				...
				case 'domain-create':
					...
					// Process Trademark extension
					if ($Domain->{'info-tm-name'})
					{
						if (!$Domain->{'info-tm-number'})
						{
							throw new Exception('Registration number field is required when registering domain with trademark');
						}
						
						$extension .= '<trademark:create ' 
									. ' xmlns:trademark="urn:afilias:params:xml:ns:ext:info-trademark-1.0"'
									. ' xsi:schemaLocation="urn:afilias:params:xml:ns:ext:info-trademark-1.0 info-trademark-1.0.xsd">'
									. '<trademark:name>'.$this->EscapeXML($Domain->{'info-tm-name'}).'</trademark:name>';
									
						if ($Domain->{'info-tm-country'})
						{
							$extension .= '<trademark:country>'.$this->EscapeXML($Domain->{'info-tm-country'}).'</trademark:country>';
						}
						$extension .= '<trademark:number>'.$this->EscapeXML($Domain->{'info-tm-number'}).'</trademark:number>';
						if ($Domain->{'info-tm-date'})
						{
							$extension .= '<trademark:date>'.$this->EscapeXML($Domain->{'info-tm-date'}).'</trademark:date>';
						}
							
						$extension .= '</trademark:create>';
					}
					
					$period = func_get_arg(4);
					$data['period'] = isset($period) ? '<domain:period unit="y">'.$period.'</domain:period>' : '';
					
					
					break;
			}
			
			$data['ext'] = $extension ? "<extension>{$extension}</extension>" : '';
		}
		...
		public function GetRemoteDomain (Domain $Domain)
		{
			...
			{
				// Trademark
				$infData = $Resp->RawResponse->response->extension->children('urn:afilias:params:xml:ns:ext:info-trademark-1.0');
				if ($infData = $infData[0])
				{
					$Resp->SetExtraField('info-tm-name', (string)$infData->name);
					$Resp->SetExtraField('info-tm-country', (string)$infData->country);					
					$Resp->SetExtraField('info-tm-number', (string)$infData->number);
					$Resp->SetExtraField('info-tm-date', (string)$infData->date);
				}
				
				// IDN
				$infData = $Resp->RawResponse->response->extension->children('urn:iana:xml:ns:idn');
				if ($infData = $infData[0])
				{
					$Resp->SetExtraField('idn-script', (string)$infData->script);
				}
			}
	...
	class AfiliasRegistryModule_OteTestSuite extends OteTestSuite
	{
