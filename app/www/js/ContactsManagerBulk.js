	...
	function SetContact()
	{
		...
		$('loader_'+TLD+'_'+type).style.display = "";
		
		var nm = 0;
		for(var ind = 0; ind < $('dropdown_'+TLD+'_'+type).childNodes.length; ind++)
		{
			if ($('dropdown_'+TLD+'_'+type).childNodes[ind] && $('dropdown_'+TLD+'_'+type).childNodes[ind].tagName == 'OPTION')
				nm++;
		}

		if (nm == 1)
		{
			$('dropdown_'+TLD+'_'+type).style.display = "none";
		}
	...
	function OnEditContactGetFormSuccess(TLD, type, response)
	{
		...
		if (response.responseText)
		{
			try
			{
				$(type+'_'+TLD+'_new_contact_value').innerHTML = response.responseText;
				$(type+'_'+TLD+'_new_contact_value').innerHTML.evalScripts();
				if ($('js_'+type))
				{
	...
	function CheckContact(type, value, TLD, groupname)
	{
		if (value == '[NEW]')
		{
			...
			$('loader_'+TLD+'_'+type).show();
			
			var optgroup = $('dropdown_'+TLD+'_'+type).down('optgroup'); 
		...
		{
			$('loader_'+TLD+'_'+type).hide();
			$("hidd_"+type+"_"+TLD).value = value;

				
			$(type+'_'+TLD+'_new_contact_cont').style.display = "none";
			$(type+'_'+TLD+'_select_contact_cont').style.display = "none";
	...
	{
		$('loader_'+TLD+'_'+type).style.display = "none";
	}
	...
	function OnCheckContactSuccess(TLD, type, response)
	{
		...
		if (response.responseText)
		{
			...
			$(type+'_'+TLD+'_new_contact_cont').style.display = "";
			$(type+'_'+TLD+'_select_contact_cont').innerHTML = "";
			$(type+'_'+TLD+'_select_contact_cont').hide();
		}
	}
	...
	function OnLoadContactSuccess (TLD, type, response)
	{
		...
		if (response.responseText)
		{
			try
			{
				$(type+'_'+TLD+'_select_contact_value').innerHTML = response.responseText;
				$(type+'_'+TLD+'_select_contact_value').innerHTML.evalScripts();
				window.contact_list_TLD = TLD;
			}
			catch(err)
			{
				//alert(err);
			...
			$(type+'_'+TLD+'_select_contact_cont').style.display = "";
			$(type+'_'+TLD+'_new_contact_cont').hide();
	...
	{
		var cnt_params = $(type+'_'+TLD+'_new_contact_cont').down('form').serialize();
	...
	function OnCreateContactSuccess(TLD, type, response)
	{
		try
		{
			...
			if (response.result == true)
			{
				...
				SelectContact(data.id, title, data.type, data.groupname, TLD);
				$('hidd_' + type + '_' + TLD).value = response.data.id; 
				$(type+'_'+TLD+'_new_contact_cont').style.display = "none";
	...
	function SelectContact (clid, title, type, groupname, TLD) {
		$$('select[groupname="'+groupname+'"]').each(function (el) {
			TLD = TLD || window.contact_list_TLD;
			if (!TLD || (TLD && el.hasClassName("tld-"+TLD))) {
