<?php
	...
	/**
	 * PHP native functions like strlen, trim, substr can't correct handle utf8 strings
	 * This class support ascii strings as well as with multibyte
	 * 
	 * Usage:
	 * $Str = new String('abc');
	 * print $Str->Length();
	...
	class String
	{
		 ...
		 * @return String
		 */
		public static function Fly ($str)
		{
			if (self::$Flyweight === null)
			{
				self::$Flyweight = new String(null);
			}
			self::$Flyweight->Set($str);
		...
		{
			return preg_match('/[\x00-\x1F\x7F-\xFF]+/', $str); 
		}
		...
		{
			return $this->is_utf8 ? utf8_substr($this->string, $offset, $length) : substr($this->string, $offset, $length);
		}
		
		public function Trim ($charlist='')
		{
 			if($charlist == '' || !$this->is_utf8)
 			{
 				trim($this->string);
 			}
 			else
 			{
 				$this->LTrim($this->RTrim($charlist), $charlist);
 			}
		...
		public function LTrim ($charlist='')
		{
 			...
 			else
 			{
	 			...
	 			$this->string = preg_replace('/^['.$charlist.']+/u','',$this->string);
 			}
		...
		{
			return $this->is_utf8 ? utf8_strtolower($this->string) : strtolower($this->string);
		}
		
		public function ToUpper ()
		{
			return $this->is_utf8 ? utf8_strtoupper($this->string) : strtoupper($this->string);
		}
	}
